...
public class XsddzhubQuery extends BaseQuery {
	...
	public Map<String, Object> buildWhere() {
		...
		if(thbz!=null){//退货标志
			this.map.put("thbz",thbz);
		}
		if(ckjsbz!=null){//出库结束标志
			this.map.put("ckjsbz",ckjsbz);
		}
		if(kpjsbz!=null){//开票结束标志
			this.map.put("kpjsbz",kpjsbz);
		}
		if(fhjsbz!=null){//发货结束标志
			this.map.put("fhjsbz",fhjsbz);
		}
		if(zdsjhj!=null){//整单税价合计
			this.map.put("zdsjhj",zdsjhj);
		}
		if(sxkje!=null){//收现款金额
			this.map.put("sxkje",sxkje);
		}
